-- Copied from https://github.com/Voronchuk/hexagons/blob/2.0.2/Hexagons/Hex.elm
-- Because both that package and eeue56/elm-flat-matrix defines `(+++)` as an
-- infix operator, and Elm is buggy and can’t handle that. I’ve commented out `(+++)`
-- below, and replaced its usage with `add`.
