...
pub fn main() {
  ...
  let positions =
    all_positions
    |> list.take(case list.length(all_positions) < 100 {
      True -> 12
      False -> 1024
    })

  let max_x =
    list.fold(positions, 0, fn(max, position) { int.max(max, position.0) })

  let max_y =
    list.fold(positions, 0, fn(max, position) { int.max(max, position.1) })
